...
const updateTaskCheckList = async (req, res) => {
    try {
        ...
        if (!task.assignedTo.includes(req.user._id) && req.user.role !== "admin") {
            return res.status(403).json({ message: "Not authorized to updtae checklist" });
        }

        task.toDoCheckList = toDoCheckList;  // replaced with updated checklist

        // auto update progress upon checklist completion
        const completedCount = task.toDoCheckList.filter(
            (item) => item.completed
        ).length;
        ...
        res.json({ message: "Task checklist updtaed", task: updatedTask });
    } catch (error) {
        res.status(500).json({ message: "Server error", error: error.message })
    }
}
